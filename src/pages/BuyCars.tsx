      ...
      <div className="bg-gradient-to-r from-[#00140F] via-[#00251B] to-[#001F17] border-b border-[rgba(180,255,210,0.12)] text-[#F2F7F3] py-10 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto space-y-2 relative z-10">
          <Reveal direction="up" distance={15}>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#00E878]/10 border border-[#00E878]/30 text-[#00E878] text-xs font-bold uppercase tracking-wider mb-2">
              Quality Selection
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-[calc(7rem+env(safe-area-inset-bottom,0px))] lg:pb-8 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <aside className="hidden lg:block lg:col-span-3 space-y-6">
            <div className="bg-[#00251B]/90 backdrop-blur-md rounded-3xl p-6 border border-[rgba(180,255,210,0.15)] shadow-glass sticky top-28 space-y-6">
              <div className="flex items-center justify-between border-b border-[rgba(180,255,210,0.12)] pb-4">
                <div className="flex items-center gap-2 text-sm font-extrabold text-[#F2F7F3]">
                  <SlidersHorizontal className="w-4 h-4 text-[#00E878]" />
                  <span>Filter Vehicles</span>
                </div>
                <button
                  onClick={resetFilters}
                  className="text-xs font-bold text-[#00E878] hover:text-[#55FF78] hover:underline flex items-center gap-1 transition-colors"
                >
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-[#8EA79C] uppercase tracking-wider mb-1">Make</label>
                  <select
                    value={make}
                    onChange={(e) => setMake(e.target.value)}
                    className="w-full rounded-xl border border-[rgba(180,255,210,0.2)] bg-[#001F17] p-2.5 text-xs font-bold text-[#F2F7F3] focus:outline-none focus:ring-2 focus:ring-[#00E878]"
                  >
                    <option value="" className="bg-[#001F17] text-[#F2F7F3]">All Makes</option>
                    ...
                    <option value="Volvo" className="bg-[#001F17] text-[#F2F7F3]">Volvo</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-[#8EA79C] uppercase tracking-wider mb-1">Transmission</label>
                  <select
                    value={transmission}
                    onChange={(e) => setTransmission(e.target.value as TransmissionType)}
                    className="w-full rounded-xl border border-[rgba(180,255,210,0.2)] bg-[#001F17] p-2.5 text-xs font-bold text-[#F2F7F3] focus:outline-none focus:ring-2 focus:ring-[#00E878]"
                  >
                    <option value="" className="bg-[#001F17] text-[#F2F7F3]">All Transmissions</option>
                ...
                <div>
                  <label className="block text-xs font-bold text-[#8EA79C] uppercase tracking-wider mb-1">Fuel Type</label>
                  <select
                    value={fuelType}
                    onChange={(e) => setFuelType(e.target.value as FuelType)}
                    className="w-full rounded-xl border border-[rgba(180,255,210,0.2)] bg-[#001F17] p-2.5 text-xs font-bold text-[#F2F7F3] focus:outline-none focus:ring-2 focus:ring-[#00E878]"
                  >
                    <option value="" className="bg-[#001F17] text-[#F2F7F3]">All Fuel Types</option>
          ...
          <main className="lg:col-span-9 space-y-6">
            ...
            {/* Top Toolbar */}
            <div className="bg-[#00251B]/90 backdrop-blur-md p-4 rounded-2xl border border-[rgba(180,255,210,0.15)] shadow-glass flex items-center justify-between gap-4">
              <div className="text-xs font-extrabold text-[#F2F7F3]">
                Showing <span className="text-[#00E878]">{vehicles.length}</span> Verified Vehicles
              </div>

              <div className="flex items-center gap-3">
                <button
                  onClick={() => setMobileFilterOpen(true)}
                  className="lg:hidden px-3 py-2 rounded-xl bg-[#001F17] border border-[rgba(180,255,210,0.2)] text-xs font-bold text-[#F2F7F3] flex items-center gap-1.5 hover:border-[#00E878]"
                >
                  <Filter className="w-3.5 h-3.5 text-[#00E878]" />
                  <span>Filters</span>
                </button>

                <div className="flex items-center gap-1.5 text-xs font-bold text-[#F2F7F3]">
                  <ArrowUpDown className="w-3.5 h-3.5 text-[#00E878]" />
                  <select
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value)}
                    className="rounded-xl border border-[rgba(180,255,210,0.2)] bg-[#001F17] p-2 text-xs font-bold text-[#F2F7F3] focus:outline-none focus:ring-2 focus:ring-[#00E878]"
                  >
                    <option value="newest" className="bg-[#001F17] text-[#F2F7F3]">Newest First</option>
                    <option value="price_low" className="bg-[#001F17] text-[#F2F7F3]">Price: Low to High</option>
                    <option value="price_high" className="bg-[#001F17] text-[#F2F7F3]">Price: High to Low</option>
                    <option value="mileage_low" className="bg-[#001F17] text-[#F2F7F3]">Lowest Mileage</option>
                  </select>
                </div>
              </div>
            </div>
              ...
              {mobileFilterOpen && (
                <div className="fixed inset-0 z-50 lg:hidden flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                  <motion.div
                    ...
                    className="bg-[#00251B] border border-[rgba(180,255,210,0.2)] rounded-3xl p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto"
                  >
                    <div className="flex items-center justify-between border-b border-[rgba(180,255,210,0.12)] pb-3">
                      <h3 className="text-base font-extrabold text-[#F2F7F3]">Filters</h3>
                      <button
                        onClick={() => setMobileFilterOpen(false)}
                        className="text-xs font-bold text-[#8EA79C] hover:text-[#F2F7F3]"
                      >
                        Close
                      </button>
                    </div>
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs font-bold text-[#8EA79C] mb-1">Make</label>
                        <select
                          value={make}
                          onChange={(e) => setMake(e.target.value)}
                          className="w-full rounded-xl border border-[rgba(180,255,210,0.2)] bg-[#001F17] p-2.5 text-xs font-bold text-[#F2F7F3]"
                        >
            ...
            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {[1, 2, 3, 4, 5, 6].map((n) => (
                  <div key={n} className="h-96 rounded-3xl bg-[#00251B]/60 animate-pulse border border-[rgba(180,255,210,0.08)]" />
                ))}
              </div>
            ) : vehicles.length === 0 ? (
              <div className="bg-[#00251B]/90 backdrop-blur-md rounded-3xl p-12 text-center border border-[rgba(180,255,210,0.15)] space-y-4">
                <Search className="w-12 h-12 text-[#00E878] mx-auto opacity-40" />
                <h3 className="text-lg font-extrabold text-[#F2F7F3]">No Vehicles Found</h3>
                <p className="text-xs text-[#8EA79C]">Try broadening your filter criteria or reset your filters.</p>
