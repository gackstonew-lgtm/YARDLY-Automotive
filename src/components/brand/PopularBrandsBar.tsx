  ...
  },
  {
    ...
    svgLogo: (
      <svg viewBox="0 0 120 60" className="w-10 h-5 sm:w-12 sm:h-6 shrink-0" fill="none">
        <ellipse cx="60" cy="30" rx="54" ry="24" fill="#014196" stroke="currentColor" strokeWidth="3" />
...
export const PopularBrandsBar: React.FC = () => {
  ...
  return (
    <div className="w-full bg-[#F4F8F6] dark:bg-[#0A0A0A] border-b border-[rgba(0,60,40,0.08)] dark:border-[rgba(255, 255, 255,0.1)] py-3.5 px-2 sm:px-4 overflow-hidden relative z-30 select-none">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-3 mb-2 px-2">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-[#0251B8] dark:bg-[#2D7DFF] animate-pulse shadow-[0_0_8px_#0251B8] dark:shadow-[0_0_8px_#2D7DFF]" />
        ...
        <button
          ...
          className="text-[11px] font-extrabold text-[#0251B8] dark:text-[#2D7DFF] hover:text-[#014196] dark:hover:text-[#FF3B4E] hover:underline transition-colors focus:outline-none"
        >
          View All Makes →
        </button>
      </div>
          ...
          {tickerItems.map((brand, idx) => {
            ...
            return (
              <button
                ...
                className={`group flex items-center gap-2.5 px-3.5 py-2.5 rounded-2xl border transition-all duration-200 shrink-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#0251B8] dark:focus:ring-[#2D7DFF] ${
                  isActive
                    ? 'bg-[#0251B8] dark:bg-[#2D7DFF] text-white dark:text-[#050505] border-[#0251B8] dark:border-[#2D7DFF] shadow-md scale-105 font-black'
                    : 'bg-white dark:bg-[#121212] text-[#0F241C] dark:text-[#F2F7F3] border-[rgba(0,60,40,0.12)] dark:border-[rgba(255, 255, 255,0.15)] hover:border-[#0251B8] dark:hover:border-[#2D7DFF] hover:bg-[#EBF2EE] dark:hover:bg-[#1A1A1A] shadow-xs active:scale-95'
                }`}
              >
                <div className={`${isActive ? 'text-white dark:text-[#050505]' : 'text-[#0251B8] dark:text-[#2D7DFF] group-hover:scale-110'} transition-transform duration-200 flex items-center`}>
                  {brand.svgLogo}
                </div>
                <div className="flex flex-col text-left leading-tight">
                  <span className={`text-xs font-black tracking-tight ${isActive ? 'text-white dark:text-[#050505]' : 'text-[#0F241C] dark:text-[#F2F7F3]'}`}>
                    {brand.name}
                  </span>
                  {brand.count && (
                    <span className={`text-[9px] font-extrabold ${isActive ? 'text-white/80 dark:text-[#050505]/80' : 'text-[#5F7E71] dark:text-[#8EA79C]'}`}>
                      {brand.count}
                    </span>
                  )}
                </div>
