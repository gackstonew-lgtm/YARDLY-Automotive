...
export const MobileBottomNav: React.FC = () => {
  ...
  const items = [
    {
      ...
      icon: (isActive: boolean) => (
        <svg className={`w-5 h-5 ${isActive ? 'text-white dark:text-[#050505]' : 'text-[#0251B8] dark:text-[#2D7DFF]'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-white/95 dark:bg-[#050505]/95 backdrop-blur-lg border-t border-[rgba(0,60,40,0.08)] dark:border-[rgba(255, 255, 255,0.15)] shadow-[0_-4px_25px_rgba(0,0,0,0.08)] dark:shadow-[0_-4px_25px_rgba(0,0,0,0.7)] pb-[env(safe-area-inset-bottom,0px)]">
          ...
          return (
            ...
            >
              ...
              <div className={`w-11 h-11 rounded-full flex items-center justify-center transition-all duration-200 group-active:scale-95 ${
                isActive 
                  ? 'bg-[#0251B8] dark:bg-[#2D7DFF] shadow-md shadow-[#0251B8]/25 dark:shadow-[#2D7DFF]/30' 
                  : 'bg-[#EBF2EE] dark:bg-[#121212] hover:bg-[#D8E6DE] dark:hover:bg-[#1A1A1A]'
              }`}>
                {item.icon(isActive)}
              </div>
              ...
              <span className={`text-[11px] font-extrabold mt-1 tracking-tight transition-colors whitespace-nowrap ${
                isActive ? 'text-[#0251B8] dark:text-[#2D7DFF]' : 'text-[#5F7E71] dark:text-[#8EA79C] group-hover:text-[#0F241C] dark:group-hover:text-[#F2F7F3]'
              }`}>
                {item.name}
              </span>
            </Link>
