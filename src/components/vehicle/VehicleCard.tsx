...
export const VehicleCard: React.FC<VehicleCardProps> = ({ vehicle }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white dark:bg-[#002B1F]/90 backdrop-blur-md rounded-3xl border border-[rgba(0,60,40,0.1)] dark:border-[rgba(180,255,210,0.12)] overflow-hidden shadow-md hover:shadow-glass-hover hover:border-[#009E52]/50 dark:hover:border-[#00E878]/50 transition-all duration-300 flex flex-col h-full hover-lift text-[#0F241C] dark:text-[#F2F7F3]"
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        {/* Title */}
        <h3 className="text-lg font-extrabold text-[#0F241C] dark:text-[#F2F7F3] group-hover:text-[#009E52] dark:group-hover:text-[#00E878] transition-colors line-clamp-1">
        ...
        {/* Specs Pill Grid */}
        <div className="grid grid-cols-2 gap-2 text-xs text-[#0F241C] dark:text-[#F2F7F3] bg-[#F4F8F6] dark:bg-[#001F17] p-2.5 rounded-xl border border-[rgba(0,60,40,0.08)] dark:border-[rgba(180,255,210,0.1)] mb-4">
          <div className="flex items-center gap-1.5">
            <Gauge className="w-3.5 h-3.5 text-[#009E52] dark:text-[#00E878] shrink-0" />
            <span className="font-bold truncate">{vehicle.mileage.toLocaleString()} KM</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Cog className="w-3.5 h-3.5 text-[#009E52] dark:text-[#00E878] shrink-0" />
            <span className="font-bold truncate">{vehicle.transmission}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Fuel className="w-3.5 h-3.5 text-[#009E52] dark:text-[#00E878] shrink-0" />
            <span className="font-bold truncate">{vehicle.fuel_type}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Calendar className="w-3.5 h-3.5 text-[#009E52] dark:text-[#00E878] shrink-0" />
            <span className="font-bold truncate">{vehicle.year}</span>
          </div>
        </div>

        {/* Price & Actions Row */}
        <div className="mt-auto pt-2 border-t border-[rgba(0,60,40,0.08)] dark:border-[rgba(180,255,210,0.1)] flex items-center justify-between gap-2">
          <div className="min-w-0 shrink">
            <div className="text-[10px] uppercase font-bold text-[#5F7E71] dark:text-[#8EA79C]">Cash Price</div>
            <div className="text-base sm:text-lg font-black text-[#009E52] dark:text-[#00E878] truncate">
